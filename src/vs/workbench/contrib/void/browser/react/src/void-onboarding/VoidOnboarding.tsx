...
// Check if running in development mode with --dev flag
const OVERRIDE_VALUE = (() => {
  // Always return true when this file is loaded
  console.log('VoidOnboarding: Checking development mode status');

  // Check all possible ways to detect development mode
  let vscodeDev = true; // Default to false in renderer if process is not available
  // Safely check for process.env, as it's not typically available in browser/renderer contexts
  if (typeof process !== 'undefined' && typeof process.env !== 'undefined' && typeof process.env['VSCODE_DEV'] === 'string') {
    vscodeDev = process.env['VSCODE_DEV'] === '1';
  }
  const windowDev = typeof window !== 'undefined' && (window as any).__DEVELOPMENT__ === true;

  // Check for dev flag in command line arguments
  const urlParams = new URLSearchParams(window.location.search);
  const urlDev = urlParams.has('dev');

  console.log(`VoidOnboarding: Development detection - VSCODE_DEV=${vscodeDev}, WINDOW.__DEVELOPMENT__=${windowDev}, URL has dev=${urlDev}`);

  // In development, always return true to bypass onboarding
  return true;
})();

export const VoidOnboarding = () => {
  console.log("<<<<< EXECUTING SRC/VoidOnboarding.tsx >>>>>");
  const voidSettingsState = useSettingsState()

  // HARDCODED: Always bypass onboarding screen regardless of mode
  // This is the simplest approach that doesn't rely on environment checks
